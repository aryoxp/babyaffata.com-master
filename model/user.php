...
class model_user extends model {
	...
	public function authenticate( $username, $password ) {
		...
		$sql = "SELECT id, name, email, level , 
					(SELECT description FROM coms_level WHERE coms_level.level=coms_user.level) as `role`,
					 (SELECT group_concat(module) FROM coms_level_module WHERE coms_user.level=level) as `module`
				FROM coms_user 
	...
	public function getList($page = 1, $perpage = 50) {
		...
		$sql = "SELECT id, username, password, name, email, level as `levelid`, status, 
				(SELECT description FROM coms_level WHERE level=coms_user.level) as `level` FROM coms_user "
	...
	public function getLevel() {
		...
		$sql 	= "SELECT level, description FROM coms_level ORDER BY level ASC";
